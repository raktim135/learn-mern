...
// Function to make the request for each month
async function makeRequests() {
    // const walletAddress = "0x53cea486bbd310b251b7b8a3ccb618199879feae";
    const walletAddress = "0x25cc275cfe3cce1700e816e00d4cd1f60872038a";
    ...
    const requests1 = dateRanges.map(async dateRange => {
        ...
        try {
            ...
            console.log(`Error for ${startDate.slice(0, 7)}: ${error}`);
        }
    });
    ...
    try {
        ...
        allTransactions.map(transaction => {
            ...
            if(token){
                ...
                const tempVol = Number(transaction.volume);
                if(!isNaN(tempVol)){
        ...
        .then((result) => {
            // console.log(result.data.ETH.USD)
            const ethPrice = result.data.ETH.USD;
            let tempTotalUsdValue = 0;
            let formatedTransactionData = [];
            allTransactions.map(transaction => {
                // console.log(transaction.nativeToken)
                let txnEthValue = 0;
            ...
            console.log("Total txns: ", formatedTransactionData.length)
            console.log("Total txns volume: ", tempTotalUsdValue)

            return formatedTransactionData;
        }).catch((err) => {
            console.log(err)
        });
        return [];
...
// Call the function to make requests for each month
makeRequests();